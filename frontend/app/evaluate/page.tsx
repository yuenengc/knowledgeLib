    ...
    setActiveStages((current) => {
      ...
      for (const item of payload.details) {
        ...
        if (!next[item.id] || !stages.includes(next[item.id])) {
          next[item.id] =
            stages.find((stage) => stageOrder.includes(stage)) ?? stages[0];
        }
      }
      return next;
    });
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
                Reads `evaluate/testset.json`, calls the main retrieval
                pipeline, and shows Precision@K, Recall@K, MRR, plus per-query
                stage-level hit details.
              </p>
            </div>
            <button
          ...
          <>
            ...
            <section className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
                ...
                <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-slate-400">
                      {payload.details.length} items
                    </div>
                  </div>
                  <div className="text-xs text-slate-500">
                    Backend-driven stage K
                  </div>
                </div>
                      ...
                      >
                        ...
                        <div className="mt-3 flex gap-2 text-[11px] text-slate-400">
                          <span className="rounded-full bg-white/5 px-2 py-1">
                            M {item.matched_count}
                          </span>
                          <span className="rounded-full bg-white/5 px-2 py-1">
                            R {item.relevant_count}
                          </span>
                        </div>
                      </button>
                    );
              ...
              <section className="min-w-0">
                {(() => {
                  const item =
                    payload.details.find(
                      (entry) => entry.id === selectedQueryId,
                    ) || payload.details[0];
                  if (!item) return null;
                  return (
                          ...
                          <div className="mt-3 grid gap-4 md:grid-cols-2">
                            <div>
                              ...
                              <ul className="mt-2 space-y-2 text-sm text-slate-200">
                                {item.matched_golden_fragments.map(
                                  (fragment) => (
                                    <li
                                      key={fragment}
                                      className="rounded-xl bg-emerald-500/10 px-3 py-2"
                                    >
                                      {fragment}
                                    </li>
                                  ),
                            ...
                            <div>
                              ...
                              <ul className="mt-2 space-y-2 text-sm text-slate-200">
                                {item.missed_golden_fragments.map(
                                  (fragment) => (
                                    <li
                                      key={fragment}
                                ...
                                <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
                                  ...
                                  {metrics && (
                                    <div className="flex flex-wrap gap-2 text-xs text-slate-300 md:justify-end">
                                      ...
                                      >
                                        P@{metrics.k} ={" "}
                                        {metrics.precision_at_k.toFixed(4)}
                                      </span>
                                      <span
                                        className="rounded-full bg-white/5 px-3 py-1"
                                        title={metricTooltip("recall", {
                                    ...
                                    >
                                      <div className="flex flex-wrap gap-2 text-xs text-slate-300">
                                        ...
                                        {hit.rerank_score !== undefined && (
                                          <span>
                                            Rerank {String(hit.rerank_score)}
                                          </span>
                                        )}
                                        {hit.file_name && (
                                          <span>{hit.file_name}</span>
                                        )}
                                        {hit.chunk_id && (
                                      ...
                                      {hit.matched && (
                                        <div className="mt-2 text-xs text-emerald-200">
                                          Best match: {hit.best_match_fragment}{" "}
                                          ({hit.best_match_ratio?.toFixed(4)})
                                        </div>
                                      )}
                                        ...
                                        {hit.matches.map((m) => (
                                          ...
                                          >
                                            <div className="font-medium">
                                              {m.matched ? "Matched" : "Missed"}
                                            </div>
                                            <div className="mt-1">
                                              fragment: {m.golden_fragment}
                                            </div>
                                            <div className="mt-1">
                                              ratio: {m.best_ratio.toFixed(4)}
                                            </div>
                                          </div>
                                        ))}
                                      </div>
